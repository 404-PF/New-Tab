...
describe('storage bridge write generations', () => {
  it('reports stale write errors without rolling back the newer generation', async () => {
    ...
    try {
      ...
      expect(dom.window.localStorage.getItem('testKey')).toBe('first');
    } finally {
      dom.window.close();
    }
  });
    ...
    try {
      ...
      dom.window.addEventListener('storageBridgeWriteError', (event) => {
        ...
        if (event.detail.generation === 2) {
          expect(dom.window.localStorage.setItem('testKey', 'third')).toBe(true);
        }
      });
      ...
      expect(dom.window.localStorage.getItem('testKey')).toBe('third');
