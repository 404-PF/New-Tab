...
describe('storage bridge', () => {
  ...
  it('resolves setItemAsync false on asynchronous chrome.storage write failures', async () => {
    const dom = new JSDOM('<!doctype html><html><body></body></html>', {
      url: 'https://example.com',
      runScripts: 'dangerously'
    });
    ...
    try {
      ...
      const writePromise = dom.window.localStorage.setItemAsync('ai_conversations', '[]');
      ...
      expect(dom.window.localStorage.getItem('ai_conversations')).toBeNull();
    } finally {
      dom.window.close();
    }
  });
