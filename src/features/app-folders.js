(function () {
  ...
  function renderFolderContents(folder, grid) {
    ...
    folder.apps.forEach(appId => {
      ...
      const safeIconUrl = window.validateIconUrl ? window.validateIconUrl(iconUrl) : iconUrl;
      a.innerHTML = `<div class="icon"><img src="${escapeHtml(safeIconUrl || '')}" alt="${escapeHtml(displayName)}" data-app-icon></div><span class="app-name">${escapeHtml(displayName)}</span>`;
