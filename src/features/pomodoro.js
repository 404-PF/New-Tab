...
(function () {
  ...
  function loadSettings() {
    try {
      ...
      return {
        enabled: parsed.enabled !== undefined ? Boolean(parsed.enabled) : DEFAULT_SETTINGS.enabled,
        work: Math.max(1, Math.floor(Number(parsed.work) || DEFAULT_SETTINGS.work)),
        shortBreak: Math.max(1, Math.floor(Number(parsed.shortBreak) || DEFAULT_SETTINGS.shortBreak)),
        longBreak: Math.max(1, Math.floor(Number(parsed.longBreak) || DEFAULT_SETTINGS.longBreak)),
        sessionsBeforeLong: Math.max(1, Math.floor(Number(parsed.sessionsBeforeLong) || DEFAULT_SETTINGS.sessionsBeforeLong))
      };
    } catch (err) {
  ...
  function sendNotification(phase, sessionsCompleted) {
    try {
      ...
      const message = i18n ? i18n.t(messageKey, { 1: String(sessionsCompleted) }) : sessionsCompleted + ' sessions completed';
